...
export const columns: ColumnDef<Medication>[] = [
...

	{
		...
		cell: ({ row }) => {
			...
			return (
				<Link
					href={`/pharmacy/dispensary/dispense?id=${Script._id}`}
					// onClick={() => setShowDispenseForm(true)}
					className='bg-[#1876f2b0] h-7 w-20 flex items-center justify-center rounded-md text-white py-px hover:bg-[#1877f2] cursor-pointer'
				>
					Dispense
				</Link>
				// <DropdownMenu>
				...
				// 			</Link>
				// 		</DropdownMenuItem>
				// 		{/* <DropdownMenuItem className="cursor-pointer">View Script History</DropdownMenuItem> */}
				// 	</DropdownMenuContent>
				// </DropdownMenu>
			);
		},
...
export function DataTableDispenseMedication() {
	...
	// const initialTabValue = useSearchParams();

	// const dispense = initialTabValue.get('dispense');

	// const isValidBooleanAndTrue = (
	...
	const [showDispenseForm, setShowDispenseForm] =
	...
	return (
		<div className='w-full'>
			...
			<Dialog
				...
				onClose={() => setShowDispenseForm(false)}
				style={{
					position: 'relative',
					zIndex: 50,
				}}
